...
export class API {
  ...
  private async request(method: string, endpoint: string, body?: unknown, params?: Record<string, unknown>): Promise<any> {
    ...
    try {
      ...
      const parse = async () => contentType.includes('application/json') ? await res.json() : await res.text()
